...
const SampleEffect = () => {
    ...
    useEffect(() => {
        let timer;
        if (timerRunning) { // timer for if our timerRunning state variable is true; the callback function in this timer logs a message to the console and resets timerRunning to false after 2 seconds expire. Because our timer resets our state variable we can now use this effects multiple times.
    ...
    useEffect(() => {
        console.log('There was a state changeeeeee baybeee')
    });
