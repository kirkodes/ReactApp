    ...
    return(
        <div className="main">
            <div className="mainDiv">
                ...
                {result ? <h2>{result.name}</h2> : <div></div>}
            </div>
        </div>
    )
}
...
const useNumHook = (num) => {
    ...
    useEffect(() => {
        if (queryNum !== '') { // checks to see if queryNum its a non empty string, if so, it fetches a math fact based on the queryNum value
    ...
    }, [queryNum]) // second argument -- queryNum inside the array, so that the callback doing the fetching is only called when theres a change to the queryNum
